...
<link rel="stylesheet" href="/timesheet-app/resource/style.css"
	type="text/css">
</head>
<body>
	<h2>Task info</h2>
		...
		<ul>
			...
			<li><label for="employees">Employees:</label>
				<table id="employees" class="task-table">
					<c:forEach items="${task.assignedEmploees}" var="emp">
						<tr>
							<sf:form action="${task.id}/employees/${emp.id}" method="delete">
								<td><a href="../employees/${emp.id}" id="href-${emp.id}">${emp.name}</a></td>
								<td><input type="submit" value="Remove"
									id="remove-${emp.id}" /> <script
										src="/timesheet-app/resources/jquery-1.7.1.js"><</script> <script
...
$("#remove-${emp.id}").on("click",function(){
	...
	$("#selected-emp").append(opt);
			...
			<li><label for="unassigned">Unassigned:</label>
