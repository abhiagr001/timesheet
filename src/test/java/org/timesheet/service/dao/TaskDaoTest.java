...
public class TaskDaoTest extends DomainAwareBase {
	...
	public void testUpdate() {
		...
		//update task
		task.setDescription("Updated Desccription");
		taskDao.update(task);
		
		Task found=taskDao.find(task.getId());
		assertEquals("Updated Desccription", found.getDescription());
	}
	...
	private Task newTaskFromTemplate(Task templateTask,String randomInfo) {
		...
		for(Employee templateEmployee : templateEmployees) {
			Employee employee=new Employee(templateEmployee.getName()+randomInfo,templateEmployee.getDepartment()+randomInfo);
